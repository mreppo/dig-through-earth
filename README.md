...
- **10-question quiz** about geography, oceans, and antipodes, with immediate ✓ / ✗ feedback and a tiered end-screen message.
...
5. **Each `quiz.questions.<id>.options` array must have exactly 4 strings, in the same order as EN.** The correct-answer index lives in `js/quiz.js`, keyed off position - reordering breaks the quiz.
...
- Hierarchical, dot-separated: `header.title`, `quiz.questions.latvia.text`, `results.distanceThrough`.
